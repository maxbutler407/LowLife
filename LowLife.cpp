...
void StartBattle(int delay1, Player& player, Enemy& enemy, const std::string& playerName, std::string character, bool transformed, bool hando) {
    while (player.health > 0 && enemy.health > 0) {
        if (character == "wizard" || character == "Wizard") {
            ...
            std::cout << "\n|        Mind Trick        ||     Sinister Spell       |";
            std::cout << "\n|       Self Enchant       ||       \033[31mDark Forces\033[0m        |";
            ...
            if (battleChoice == "Mind Trick" || battleChoice == "Mind trick" || battleChoice == "mind trick") {
                player.MindTrick(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else if (battleChoice == "Sinister Spell" || battleChoice == "Sinister spell" || battleChoice == "sinister spell") {
                player.SinisterSpell(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else if (battleChoice == "Self Enchant" || battleChoice == "Self enchant" || battleChoice == "self enchant") {
                player.SelfEnchant(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else if (battleChoice == "Dark Forces" || battleChoice == "Dark forces" || battleChoice == "dark forces") {
                player.DarkForces(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else if (battleChoice == "Lightning Wrath" || battleChoice == "Lightning wrath" || battleChoice == "lightning wrath") {
                player.LightningWrath(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else if (battleChoice == "Lather Ectoplasm" || battleChoice == "Lather ectoplasm" || battleChoice == "lather ectoplasm") {
                player.LatherEctoplasm(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else {
                std::cout << "\n\033[90mPlease specify an action!\033[0m\n";
            }
            ...
            if (!transformed) {
                ...
                std::string battle2 = "\n\nWhat would you like to do?";
                Stagger(battle2, delay1);
                std::cout << "\n|        \033[31mTransform\033[0m        ||       Throw Stone        |";
                if (player.headCount >= 1) {
                    std::cout << "\n          |        Lightning Wrath         |";
                }
                else if (player.ectoplasmCount >= 1) {
                ...
                else if (battleChoice == "Throw Stone" || battleChoice == "Throw stone" || battleChoice == "throw stone") {
                ...
                else if (transformChoice == "Fearful Roar" || transformChoice == "Fearful roar" || transformChoice == "fearful roar") {
                    player.FearfulRoar(delay1, playerName, enemy.enemyName, player, enemy);
                }
                else if (transformChoice == "Beat Down" || transformChoice == "Beat down" || transformChoice == "beat down") {
                    player.BeatDown(delay1, playerName, enemy.enemyName, player, enemy);
                }
                else if (transformChoice == "Claw Stab" || transformChoice == "Claw stab" || transformChoice == "claw stab") {
                    player.ClawStab(delay1, playerName, enemy.enemyName, player, enemy);
                }
                else if (transformChoice == "Lightning Wrath" || transformChoice == "Lightning wrath" || transformChoice == "lightning wrath") {
                    player.LightningWrath(delay1, playerName, enemy.enemyName, player, enemy);
                }
                else if (transformChoice == "Lather Ectoplasm" || transformChoice == "Lather ectoplasm" || transformChoice == "lather ectoplasm") {
                    player.LatherEctoplasm(delay1, playerName, enemy.enemyName, player, enemy);
                }
                else {
                    std::cout << "\n\033[90mPlease specify an action!\033[0m\n";
            ...
            std::string battle2 = "\n\nWhat would you like to do?";
            Stagger(battle2, delay1);
            std::cout << "\n|        Examine          ||      Optimise Moves      |";
            std::cout << "\n|      \033[31mCall The Stars\033[0m       ||  Use Telescope As Weapon   |";
            if (player.headCount >= 1) {
                std::cout << "\n          |        Lightning Wrath         |";
            }
            else if (player.ectoplasmCount >= 1) {
            ...
            else if (battleChoice == "Optimise Moves" || battleChoice == "Optimise moves" || battleChoice == "optimise moves") {
                player.OptimiseMoves(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else if (battleChoice == "Call The Stars" || battleChoice == "Call the stars" || battleChoice == "call the stars") {
                player.CallTheStars(delay1, playerName, enemy.enemyName, player, enemy);
            }
            else if (battleChoice == "Use Telescope As Weapon" || battleChoice == "Use telescope as weapon" || battleChoice == "use telescope as weapon") {
    ...
    if (player.health > 0) {
        ...
        stopMusic();
        startMusic("LowLifeMusic/travel.wav");
    }
...
void InitialiseBattle(int delay1, Player& player, const std::string& playerName, std::string character, bool randomEnemy, bool avis, bool pigman, bool bali, bool tonobros) {
    if (pigman == true) {
        stopMusic();
        startMusic("LowLifeMusic/miniBoss.wav");
        Enemy enemy;
    ...
    if (randomEnemy) {
        stopMusic();
        startMusic("LowLifeMusic/battle_music1.wav");
        Enemy enemy;
    ...
    if (tonobros == true) {
        stopMusic();
        startMusic("LowLifeMusic/battle_music3.wav");
        Enemy enemy;
...
public:
    ...
    void HermitDialogue(std::string &playerName, bool &spokenToHermit, bool tonobros) const {
        if (spokenToHermit == false) {
            ...
            DisplayPhotoInNewWindow("LowLifePhotos\\Hermit.jpeg", "Kroka, The Content Dwarf");
    ...
    void GhostBoyDialogue(std::string& playerName, bool &spokenToFilia, bool tonobros) const {
        if (spokenToFilia == false) {
            ...
            Stagger(boy1, delay1);
            DisplayPhotoInNewWindow("LowLifePhotos\\GhostBoy.jpeg", "Filia, The Ghost-Child");
            Stagger(boy2, delay1);
            Stagger(boy3, delay1);
            Stagger(boy4, delay1);
            spokenToFilia = true;
    ...
    void RottenPigmanDialogue(std::string& playerName, int delay1, Player& player, std::string character, bool randomEnemy, bool avis, bool pigman, bool bali, bool &spokenToGorkan, bool tonobros) const {
        if (spokenToGorkan == false) {
            ...
            DisplayPhotoInNewWindow("LowLifePhotos\\Pigman.jpeg", "Gorkan, The Rotten Pigman");
            ...
            std::string gorkan4 = "\nAlthough the stories of heroes are long dead, I believe you could be significant in re-establishing the \033[33mMoranthian Order\033[0m in this land and that wretched other.";
    ...
    void LordBaliDialogue(std::string& playerName, int delay1, Player& player, std::string character, bool randomEnemy, bool avis, bool pigman, bool bali, bool &spokenToBali, bool tonobros) const {
        if (spokenToBali == false) {
            ...
            DisplayPhotoInNewWindow("LowLifePhotos\\LordBali.jpeg", "Lord Bali, Khan of the Kaltorna Galaxy");
            while (true) {
                std::string bali3 = "\nWhy does this LowLife come to me?";
                Stagger(bali3, delay1);
                std::string choice1;
                std::cout << "\nTo Fight | To Submit | To Bargain";
                std::cout << "\n> ";
                std::getline(std::cin, choice1);

                if (choice1 == "To Fight" || choice1 == "To fight" || choice1 == "to fight") {
                    ...
                    DisplayPhotoInNewWindow("LowLifePhotos\\LordBali3.jpeg", "Lord Bali, Khan of the Kaltorna Galaxy");
                ...
                else if (choice1 == "To Submit" || choice1 == "To submit" || choice1 == "to submit") {
                ...
                else if (choice1 == "To Bargain" || choice1 == "To bargain" || choice1 == "to bargain") {
    ...
    void CowboyDialogue(std::string& playerName, int delay1, Player& player, std::string character, bool randomEnemy, bool avis, bool pigman, bool bali, bool& spokenToCowboy, bool tonobros) const {
        if (spokenToCowboy == false) {
            ...
            DisplayPhotoInNewWindow("LowLifePhotos\\ManOnDino.jpeg", "Hando, The Lone Explorer");
            ...
            DisplayPhotoInNewWindow("LowLifePhotos\\DesertBird.jpeg", "The Great Vermilion Avis");
    ...
    void TonobrosDialogue(std::string& playerName, int delay1, Player& player, std::string character, bool randomEnemy, bool avis, bool pigman, bool bali, bool& spokenToTonobros, bool tonobros) const {
        if (spokenToTonobros == false) {
            if (player.headCount > 0 || player.ectoplasmCount > 0) {
                ...
                DisplayPhotoInNewWindow("LowLifePhotos\\Boss.jpeg", "Tonobros, The Devourer of Worlds");
                ...
                if (player.headCount > 0 && player.darkness >= 60) {
                    stopMusic();
                    startMusic("LowLifeMusic/Finale.wav");
                    ...
                    DisplayPhotoInNewWindow("LowLifePhotos\\End3.jpeg", " ");
                    DisplayPhotoInNewWindow("LowLifePhotos\\End2.jpeg", " ");
                    ...
                    Sleep_1s();
                    DisplayPhotoInNewWindow("LowLifePhotos\\End6.jpeg", " ");
                    ...
                    DisplayPhotoInNewWindow("LowLifePhotos\\End5.jpeg", " ");
...
void PilotDialogue(std::string& playerName, int delay1, Player& player, std::string character, bool randomEnemy, bool avis, bool pigman, bool bali, bool& spokenToPilot, bool tonobros) {
    if (spokenToPilot == false) {
        ...
        DisplayPhotoInNewWindow("LowLifePhotos\\pilot.jpeg", "Hank, Some Border Control Officer");
...
public:
    ...
    bool PrintImageWhenEnteringRegion(int newRow, int newCol) {
        if (newRow == 0 && newCol == 0) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Mushrooms.jpeg", " ");
            return true;
        }
        if (newRow == 0 && newCol == 1) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Desert2.jpeg", " ");
            return true;
        }
        if (newRow == 0 && newCol == 3) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Palace.jpeg", " ");
            return true;
        }
        if (newRow == 1 && newCol == 0) {
            DisplayPhotoInNewWindow("LowLifePhotos\\BlueForest.jpeg", " ");
            return true;
        }
        if (newRow == 1 && newCol == 1) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Ravine.jpeg", " ");
            return true;
        }
        if (newRow == 1 && newCol == 2) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Portal2.jpeg", " ");
            return true;
        }
        if (newRow == 1 && newCol == 3) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Station2.jpeg", " ");
            return true;
        }
        if (newRow == 2 && newCol == 0) {
            DisplayPhotoInNewWindow("LowLifePhotos\\ForestRuins.jpeg", " ");
            return true;
        }
        if (newRow == 2 && newCol == 1) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Forest.jpeg", " ");
            return true;
        }
        if (newRow == 2 && newCol == 3) {
            DisplayPhotoInNewWindow("LowLifePhotos\\Pyramid.jpeg", " ");
            return true;
        }
    }
...
void Prologue(bool bold, int delay1, int delay2)
{
    ...
    Sleep_1s();
    std::string prologue2 = "\n\n...eventually however, a portal suddenly appeared, leading to a new frontier of science and technology...";
...
void IntroductionP2(int delay1, std::string playerName, Player& player) {
    ...
    DisplayPhotoInNewWindow("LowLifePhotos\\Falling.jpeg", "Welcome to Moranthia");
...
void InitialiseGameMap(std::string playerName, Player& player, std::string character, bool randomEnemy, bool avis, bool pigman, bool bali, bool tonobros) {
    ...
    while (running) {
        ...
        if (choice == "north") {
            ...
            std::cin.ignore(); // Ignores leftover newline character from previous input.
...
int main()
{
    // sends ALL error messages to nowhere (REMOVE WHEN DEVELOPING)
    ...
    PreText(bold, delay3);
    startMusic("LowLifeMusic/main_theme.wav");
